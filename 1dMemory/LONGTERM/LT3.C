...
int length[20]={9,12,16,20,26,33,41,50,60,72,85,100,117,136,157,180,206,234,264,298};

/*char junkr[20]={'1','2','3','4','5','6','7','8','9','0','q','w','e','r','t','y','u','i','o','p'};*/
...
main()
{
 ...
 /* set up pallete here*/
  ...
  for (s=0;s<20;s++)
  {
    ...
    sprintf(junk,"%d",s+1);
    vhline(length[s],(s*8),picBuf,&seed);
    drawText(junk,xcenter,(s*8)+0,0,255);
  }
  ...
  for (b=0;b<numblocks;b++)
      ...
      for (s=0;s<trialsperblock;s++)
	{
	  ...
	  switch (data->x[0].resp)
	    {
	     ...
	     case '@': resp=20;break;
	     default : resp=21;audio(INVALID,wrong_m);break;
	   }
	  if (resp==stim[t]) 
	    { 
	      audio(CORRECT);
	  ...
	  else
	    {
		 ...
		 case '@': second=20;break;
		 default : second=21;audio(INVALID,wrong_m);break;
	       }
	      if (second==stim[t]) 
		audio(CORRECT);
	     else
	  ...
	  if (resp==20) {fclose(outfile);CleanupMoviePackage();
