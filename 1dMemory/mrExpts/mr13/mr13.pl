...
sub createexperiment {
	...
	%stimuligroup=('A','0235689','B','0134679');
	...
	@stimulisource=("45x1-255255255.bmp",
			"55x1-255255255.bmp",
			"67x1-255255255.bmp",
			"82x1-255255255.bmp",
			"100x1-255255255.bmp",
		...
		if (substr($grouporders[$group],$i-1,1) ne '.') {
			...
			for ($j=1;$j<=$trialspergroup;$j++) {
				...
				for ($k=0;$k<=length($stimuligroup{substr($grouporders[$group], $i-1, 1)})-1;$k++){
					if (substr($keymapping, index($stimuligroup{substr($grouporders[$group], $i-1, 1)} ,substr($stimuligroup{substr($grouporders[$group], $i-1, 1)}, $k,1)),1)==$correct){
						$itemline.="<mpr +".$correct.">";
					}else{
						$itemline.="<mnr +".substr($keymapping,index($stimuligroup{substr($grouporders[$group], $i-1,1)},substr($stimuligroup{substr($grouporders[$group], $i-1, 1)}, $k, 1)),1).">";
					}
				}
				#$itemline.="<CorrectFeedback \"Correct\"><WrongFeedback \"$correct\">";
				$itemline.=" * ";
				
				$itemline.="\"$stimulisource[substr($stimuligroup{substr($grouporders[$group], $i-1,1)}, index($keymapping,$correct),1)]\" <Call -".((index($keymapping, $correct)+1)*10).">;\n";
				print ITEMFILE $itemline;
				print OUTFILE (1000+$itemcounter).",".$correct.",".substr($stimuligroup{substr($grouporders[$group], $i-1,1)}, index($keymapping,$correct),1)."\n";
			}	
		}else{
			print OUTFILE "*BREAK*\n";
