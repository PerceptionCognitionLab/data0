...
sub createexperiment {
	...
	$keymapping="12345678";
	$stimuligroups="ABCD";
	%stimuligroup=('A','0456789D','B','0123ABCD','C','0123456D','D','0789ABCD');
	...
	@grouporders=(&create_random_trials);
	$addtoseed[0]='.'x(length($grouporders[0]));
...
sub create_random_trials{
my $breaks=100;
my $trials=0;
my $w, $j, $s, $groupstring,$groupbreaks;

$w=substr($stimuligroups, int(rand(1)*length($stimuligroups)),1);
...
for ($s=0;$s<(1000/$trialspergroup);$s+=($breaks/$trialspergroup)){

$groupbreaks.=substr($groupstring,$s,($breaks/$trialspergroup)).'.';
