...
int chainlength[numchain]={18,18,18,18};
...
main()
{
  ...
  int to[totalnumber],t,c,j,i,lo[numchain][30],respnum,j1,s;
  ...
  for (t=0;t<chainlength[0];t++)
   {
    eng_pixels[t]=(char *)malloc(sizeof(char)*(82*82));
 ...
 for (t=bottomnumber;t<topnumber;t++)
  ...
  /*practice with widgets*/
  runMovie(start_m,UNTIL_RESPONSE,0);
  j=3;
  downloadImage(start_i);
  clearPicBuf();
  drawText("Widgets",xcenter,ycenter,0,255);
  uploadImage(start_i);
  setMovie(start_m,0,start_i,1);
  runMovie(start_m,UNTIL_RESPONSE,0);
  ...
  runMovie(start_m,UNTIL_RESPONSE,0);
  for (j1=0;j1<numchain-1;j1++)
    {
      j=(j1+log->subjnum)%(numchain-1);
      downloadImage(start_i);
      clearPicBuf();
      if (j==0) drawText("ENGLISH",xcenter,ycenter,0,255);
      if (j==1) drawText("GEORGIAN",xcenter,ycenter,0,255);
...
int present(int j, int a, int b, int feedback, int *respnum)
{
  ...
  if (feedback!=1) runMovie(instruct_m,UNTIL_RESPONSE,0);
  ...
  if (feedback==2)
    drawText("Study:",xcenter-50,ycenter-70,0,255);
  uploadImage(targ_i);
  setMovie(stim_m,1,targ_i,120);
  if (feedback==0)
    {
      data=runMovie(stim_m,UNTIL_RESPONSE,0);
  ...
  else runMovie(stim_m,UNTIL_RESPONSE,0);
}
