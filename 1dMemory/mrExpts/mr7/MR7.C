...
int chainlength[numchain]={18,18};
...
main()
{
  ...
  int numbers[totalnumber]; /* all response numbers used */
  int  t,c,j,i,lo[numchain][30],response;
  int respterm[numchain][30]; /*response term for each chain*/
  int stimterm[numchain][30],current[numchain];
 ...
 for (t=bottomnumber;t<topnumber;t++)
    numbers[t-bottomnumber]=t;
  distribute(numbers,totalnumber,&seed);
  c=0;
  for (j=0;j<numchain;j++)
    {
      for (i=0;i<chainlength[j];i++)
	{
          respterm[(j+log->subjnum)%numchain][i]=numbers[c];
	  stimterm[j][i]=i;
	  c++;
	}
      distribute(stimterm[j],chainlength[j],&seed);
    }
  ...
  for (j=0;j<numchain;j++)
    {
      present(j,stimterm[j][0],respterm[j][0],2,&response);
      flag[j]=0;
      current[j]=0;
      top[j]=0;
    }
  ...
  for (t=0;t<numtrials;t++)
    {
      j=t%numchain;      
      success=present(j,stimterm[j][lo[j][current[j]]],respterm[j][lo[j][current[j]]],0,&response);
      sprintf(outline,"%02d %03d %1d %02d %02d %02d %1d %02d %02d",log->subjnum,t,j,
        stimterm[j][lo[j][current[j]]],respterm[j][lo[j][current[j]]],lo[j][current[j]],success,top[j],response);
	...
	{
	  flag[j]=1;
          present(j,stimterm[j][lo[j][current[j]]],respterm[j][lo[j][current[j]]],1,&response);
	}
      current[j]++;
      ...
      if (current[j]>top[j])
	{
	  if (flag[j]==0)
	    {
	      top[j]++;
              present(j,stimterm[j][lo[j][top[j]]],respterm[j][lo[j][top[j]]],2,&response);
	    }
