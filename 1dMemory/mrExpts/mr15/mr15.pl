...
($subjectnumber,$scrseed)=@ARGV;
$randomseed=int($subjectnumber/2)+$scrseed;
die "You must specify a subject number and randomseed: $!"if (@ARGV == ());
$condition=0;
#print "Group [0]:\n";
#chomp($group=<STDIN>);
...
#####Experiment stuff
@openingmessages=(
		  "In this experiment, you will be shown characters.",
		  "Each character is associated with a number, 1-8.",
	          "For each character, press the correct number.",
...
#	print "\n$numberofendingmessages\n";
	print "S#: $subjectnumber, Rand: $randomseed\n"; 	
close OUTFILE;
close ITEMFILE;
end;
...
sub createexperiment {
	...
	@addtoseed  =("91..6..7..2..3..8..5..4..2..7..8..3..4..5..6..1..",
                      "91..6..7..2..3..8..5..4..2..7..8..3..4..5..6..1..");
        @grouporders=("AA.AB.BB.BA.AA.AB.BB.BA.AA.AB.BB.BA.AA.AB.BB.BA.A",
	...
	for ($i=1;$i<=length($grouporders[$group]);$i++) {
		...
		if ((substr($grouporders[$group],$i-1,1) ne '.')&&(substr($grouporders[$group],$i-1,1) ne '|')) {
			...
			if ($blockedrand){
				...
				push(@correctans, substr($stimuligroup{substr($grouporders[$group], $i-1,1)},int(rand() * length($stimuligroup{substr($grouporders[$group], $i-1,1)})),1));
